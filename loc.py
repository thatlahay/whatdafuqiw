...
async def on_message(message):
	...
	if message.content.startswith("$check weather") and str(message.channel) == "check-thời-tiết":
		...
		if re.search("HCM",message_upper):
			woe_id = "1252431"
		if re.search("CANDUOC",message_upper):
			woe_id = "1252350"
		if re.search("HANOI",message_upper):
			woe_id = "1236594"
		if re.search("DANANG",message_upper):
			woe_id = "1252376"
		if re.search("DONGTHAP",message_upper):
			woe_id = "2347722"
		if re.search("SINGAPORE",message_upper):
			woe_id = "1062617"
	...
	if message.content.startswith("$help"):
		await client.send_message(message.channel, """$help là hiện lên cái này nè thằng ngu =))
		$check weather "tenthanhpho"  vd: $check weather canduoc
		""")
	#Delete 100 msg in a Channel
	if message.content.startswith("$clear") and message.author.id == "481986993530732544":
		msg = []
		number = 0
